...
- `static/engine.js` — The whole runtime: `seek(t)` applies ops forward, pins every animation on the page to timeline time, runs JS tweens, then resolves anchors in one batched read phase followed by one batched write phase.
